...
int InitConf()
{
	...
	GetProfileString("MAIN", "ORACLE_USER", mdb->config.oracle_user);

	/* REDIS 정보를 가져온다 */
	mdb->config.redis_count = GetProfileInt("REDIS", "REDIS_COUNT", 0);
	for ( i = 0 ; i < mdb->config.redis_count ; i++ )
	{
		sprintf(path, "REDIS_IP_%d", i+1); GetProfileString("REDIS", path, mdb->config.redis[i].ip);
		sprintf(path, "REDIS_PORT_%d", i+1); mdb->config.redis[i].port = GetProfileInt("REDIS", path, 0);
		sprintf(path, "REDIS_AUTH_%d", i+1); GetProfileString("REDIS", path, mdb->config.redis[i].auth);
	}
	...
	Log("InitConf: 서버 환경 정보 초기화에 성공 ORACLE_USER[%s]\n", mdb->config.oracle_user);
	for ( i = 0 ; i < mdb->config.redis_count ; i++ ) Log("InitConf: 서버 환경 정보 초기화에 성공 REDIS [%s][%d][%s]\n", mdb->config.redis[i].ip, mdb->config.redis[i].port, mdb->config.redis[i].auth);
